...
#'
#' @return A character vector of split stems converted to names, with each disease in a stem separated by |.
#'
#' @examples
...
name_stems = function(split_stems, dis_names){
  ...
  named_stems = sapply(split_stems, function(x){
    ...
    named_stem = paste(dis_names[single_dis], collapse= ' | ')
    return(named_stem)
    }
    }, USE.NAMES = F)
    return(named_stems)
